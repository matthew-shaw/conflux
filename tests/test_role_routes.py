...
def test_ui_create_get_renders_form(monkeypatch, client):
    fake_form = DummyForm()
...
def test_ui_create_post_success_redirects(monkeypatch, client):
    ...
    response = client.post("/roles/new", data={})

    assert response.status_code == 302
    assert "/roles/" in response.headers["Location"]


def test_ui_create_post_duplicate_shows_error(monkeypatch, client):
    fake_form = DummyForm(submit=True, name="Role Name", grade="Grade 1")
    monkeypatch.setattr(role_ui, "RoleForm", lambda: fake_form)
    monkeypatch.setattr(
        role_ui,
...
def test_ui_edit_get_populates_form(monkeypatch, client):
    ...
    response = client.get("/roles/00000000-0000-0000-0000-000000000000/edit")
...
def test_ui_edit_post_success_redirects(monkeypatch, client):
    ...
    response = client.post("/roles/00000000-0000-0000-0000-000000000000/edit", data={})

    assert response.status_code == 302
    assert "/roles/" in response.headers["Location"]


def test_ui_edit_post_duplicate_shows_error(monkeypatch, client):
    fake_role = SimpleNamespace(id="1", name="Old name", grade="Grade 1")
    fake_form = DummyForm(submit=True, name="New name", grade="Grade 1")
    monkeypatch.setattr(role_ui, "get_role", lambda id: fake_role)
    monkeypatch.setattr(role_ui, "RoleForm", lambda: fake_form)
...
def test_ui_archive_get_renders_form(monkeypatch, client):
    ...
    response = client.get("/roles/00000000-0000-0000-0000-000000000000/archive")
...
def test_ui_archive_post_success_redirects(monkeypatch, client):
    ...
    response = client.post("/roles/00000000-0000-0000-0000-000000000000/archive", data={})

    assert response.status_code == 302
    assert "/roles/" in response.headers["Location"]


def test_ui_restore_post_success_redirects(monkeypatch, client):
    ...
    response = client.post("/roles/00000000-0000-0000-0000-000000000000/restore", data={})

    assert response.status_code == 302
    assert "/roles/" in response.headers["Location"]


def test_ui_download_returns_csv(monkeypatch, client):
    ...
    response = client.get("/roles/download")
